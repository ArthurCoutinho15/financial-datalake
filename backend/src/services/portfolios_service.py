...
from src.schemas.portfolios_schema import PortfolioSchema, PortfolioCreateSchema, PortfolioUpdateSchema
...
class PortfoliosService:
    ...
    async def put_portfolio(self, portfolio_id: UUID, portfolio: PortfolioUpdateSchema) -> PortfoliosModel:
        query = select(PortfoliosModel).where(PortfoliosModel.id == portfolio_id)
        result = await self.db.execute(query)
