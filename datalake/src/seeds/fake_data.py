...
def pick_asset() -> tuple[str, str]:
    ...
    asset_type = random.choices(["stock", "crypto", "fx"], weights=[60, 30, 10], k=1)[0]
    ticker = random.choice(TICKERS[asset_type])
    return ticker, asset_type


def simulate_price_history(
    base_price: float, n: int, volatility: float = 0.04
) -> list[float]:
...
def generate_clients(n: int) -> list[dict]:
    clients = []
    for _ in range(n):
        # Se o período é pequeno, distribui clientes ao longo de todo o período
        days_available = (END_DATE - START_DATE).days
        days_before_end = min(30, days_available) if days_available > 0 else 0
        created_at = random_date(
            START_DATE,
            END_DATE - timedelta(days=days_before_end)
...
def generate_positions_and_transactions(
    ...
    for portfolio in portfolios:
        ...
        for _ in range(n_positions):
            ...
            pos_created = portfolio["created_at"] + timedelta(
                days=random.randint(0, min(30, days_until_end))
            )
...
def write_csv(
    clients, portfolios, positions, transactions, base_path: str = "fake_data"
):
